...
import com.mr.http.MR_VolleyLog.MarkerLog;
...
public abstract class MR_Request<T> implements Comparable<MR_Request<T>> {
	 ...
	 */
	private static final String DEFAULT_PARAMS_ENCODING = MR_HttpConfig.CONFIG_WEB_CHARSET;
	 ...
	 * cache.
	 */
	private MR_Cache.Entry mCacheEntry = null;
	 ...
	 *
	 * @deprecated Use
	 *             {@link #Request(int, String, com.mr.http.MR_Response.ErrorListener)}
	 *             .
	 */
	public MR_Request(String url, MR_Response.ErrorListener listener) {
	 ...
	 * response.
	 */
	public MR_Request(int method, String url, MR_Response.ErrorListener listener) {
		mMethod = method;
		mUrl = url;
	 ...
	 * tag by {@link MR_RequestQueue#cancelAll(Object)}.
	 */
	 ...
	 */
	public void setRetryPolicy(MR_RetryPolicy retryPolicy) {
	...
	void finish(final String tag) {
		...
		if (MarkerLog.ENABLED) {
			...
			if (requestTime >= SLOW_REQUEST_THRESHOLD_MS) {
				MR_VolleyLog.d("%d ms: %s", requestTime, this.toString());
			}
		}
	}
	 ...
	 */
	public void setRequestQueue(MR_RequestQueue requestQueue) {
		mRequestQueue = requestQueue;
	}

	/**
	 * Sets the sequence number of this request. Used by {@link MR_RequestQueue}
	 * .
	 */
	 ...
	 * throw {@link MR_AuthFailureError} as authentication may be required to
	 * provide these values.
	 * 
	 * @throws MR_AuthFailureError
	 ...
	 * a simple GET should be used. Can throw {@link MR_AuthFailureError} as
...
//				encodedParams.append('&');
//			}
////			LogManager.i("HTTP上送参数",  encodedParams.toString());
...
//		}
		 ...
		 */
		try {
			LogManager.i("HTTP上送参数", MR_CommonUtil.toJson(params));
			return MR_CommonUtil.toJson(params).getBytes(paramsEncoding);
	 ...
	 * cause delivery of a {@link MR_TimeoutError} error.
	 */
	 ...
	 */
	abstract protected MR_Response<T> parseNetworkResponse(MR_NetworkResponse response);
	 ...
	 */
	protected MR_VolleyError parseNetworkError(MR_VolleyError volleyError) {
		return volleyError;
	}
	 ...
	 *            {@link #parseNetworkResponse(MR_NetworkResponse)}
	 */
	abstract protected void deliverResponse(T response, String requestType);
	 ...
	 *            Error details
	 */
	public void deliverError(MR_VolleyError error, String requestType) {
		if (mErrorListener != null) {
			mErrorListener.onErrorResponse(error, requestType);
		}
	}
	 ...
	 */
	@Override
	public int compareTo(MR_Request<T> other) {

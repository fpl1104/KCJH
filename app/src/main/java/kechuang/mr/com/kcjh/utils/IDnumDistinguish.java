...
public class IDnumDistinguish {
	...
	private String[] errMsg = new String[] { "正确身份证号", "身份证号不能为空", "身份证号长度不正确", "身份证号有非法字符", "身份证号出生日期不合法", "身份证号校验位错误" };
	 ...
	 * @param idCardNum
	 */
	public IDnumDistinguish(String idCardNum) {
		// super();
	...
	public String getErrMsg() {
		LogManager.i("this.error:"+this.error);
	...
	public int isCorrect() {
		if (this.isEmpty()) {
			this.error = IDnumDistinguish.IS_EMPTY;
			return this.error;
		}

		if (!this.isLenCorrect()) {
			this.error = IDnumDistinguish.LEN_ERROR;
			return this.error;
		}

		if (!this.isCharCorrect()) {
			this.error = IDnumDistinguish.CHAR_ERROR;
			return this.error;
		}

		if (!this.isDateCorrect()) {
			this.error = IDnumDistinguish.DATE_ERROR;
			return this.error;
		}
		if (!this.getCheck().equals(this.getCheckBit())) {
			this.error = IDnumDistinguish.CHECK_BIT_ERROR;
			return this.error;
		}
		return 0;
	}

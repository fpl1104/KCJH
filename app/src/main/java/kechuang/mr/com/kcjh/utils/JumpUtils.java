...
public class JumpUtils {
	 ...
	 * @param delay
	 */
	public static void goNextDelay(final Context context, final Class<?> to,
			long delay) {
	 ...
	 *            退出动画
	 */
	public static void goNext(Context context, Class<?> to, int enterAnim,
			int exitAnim) {
		goNext(context, to, null, null, false);
	 ...
	 */
	public static void goNextDelay(final Context context, final Class<?> to,
			long delay, final boolean finished) {
		Timer timer = new Timer();
		timer.schedule(new TimerTask() {
	 ...
	 */
	public static void goNextDelay(final Context context, final Class<?> to,
			final Object paraValues, long delay, final boolean finished) {
		Timer timer = new Timer();
		timer.schedule(new TimerTask() {
	 ...
	 *            父Activity是否销毁，finished=true销毁
	 */
	public static void goNext(Context context, Class<?> to, Object paraValues,
			boolean finished) {
		goNext(context, to, null, paraValues, finished);
	}
	 ...
	 */
	public static void goNextDelay(final Context context, final Class<?> to,
			final String paraName, final Object paraValues, long delay,
	 ...
	 * @param finished
	 */
	public static void goNext(Context context, Class<?> to, String paraName,
			Object paraValues, boolean finished) {
	 ...
	 *            request code
	 */
	public static void invokeActivityDelay(final Context context,
			final Class<?> to, long delay) {
		Timer timer = new Timer();
		timer.schedule(new TimerTask() {
	 ...
	 */
	public static void invokeActivityDelay(final Context context,
			final Class<?> to, final String paraName, final Object paraValues,
			long delay) {
		Timer timer = new Timer();
		timer.schedule(new TimerTask() {
	 ...
	 */
	public static void invokeActivity(Context context, Class<?> to,
			String paraName, Object paraValues) {
	 ...
	 */
	public static void invokeActivity(Context context, Class<?> to,
			String paraName, Object paraValues, int requestCode) {
		Intent intent = new Intent();
		intent.setClass(context, to);
		if (paraValues != null)
			initParas(intent, paraName, paraValues);
	...
	/**
	 * 执行隐式意图操作，处理完成后可以返回值，实现类中需要重载方法：onActivityResult(int requestCode, int
	...
	protected static void initParas(Intent intent, String parasName,
			Object value) {
		if (value == null) {
			return;
		}
		...
		if (value instanceof Integer) {
			...
			intent.putExtra(parasName, value.toString());

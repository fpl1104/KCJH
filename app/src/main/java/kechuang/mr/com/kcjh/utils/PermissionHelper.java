...
public class PermissionHelper {
	...
	private void checkCallingObjectSuitability(@Nullable Object object) {
		...
		if (!(isSupportFragment || isActivity || (isAppFragment && isNeedRequest()))) {
			...
			// if (isAppFragment) {
			// throw new
			// IllegalArgumentException("Target SDK needs to be greater than 23 if caller is android.app.Fragment");
			// } else {
			// throw new
			// IllegalArgumentException("Caller must be an Activity or a Fragment.");
			// }
		}
	}
	...
	public void showMessageOKCancel(CharSequence message, DialogInterface.OnClickListener okListener) {
		new AlertDialog.Builder(getActivity(mContext)).setMessage(message).setPositiveButton("确认", okListener)
